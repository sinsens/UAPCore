  ...
  <div class="app-container">
    <div class="head-container">
      ...
      <el-select v-model="listQuery.status" @change="handleFilter">
        <el-option v-for="item in lockerStatusOptions" :key="item.id" :value="item.id" :label="item.name" />
      </el-select>
    ...
    <el-dialog :visible.sync="dialogFormVisible" :close-on-click-modal="false" :title="formTitle" @close="cancel()"
      ...
      <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="100px">
        <el-form-item label="所属应用" prop="appId">
          <el-select v-model="form.appId" style="width: 188px;" placeholder="请选择"
        ...
        <el-form-item label="状态" prop="status">
          <el-select v-model="form.status" style="width: 188px;">
            <el-option v-for="item in lockerStatusOptions" :key="item.name" :label="item.name" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
      </el-form>
      <div style="text-align: right;">
        ...
        <el-button size="small" v-loading="formLoading" type="primary" @click="save" :v-permission="[
      ...
      <div slot="footer" class="dialog-footer" v-if="showRentList">
        <el-table :data="rentList" stripe style="width: 100%;" row-key="id"
          :default-sort="{ prop: 'rentTime', order: 'descending' }">
          <el-table-column prop="rentTime" label="起租时间" width="180" :formatter="formatTime"></el-table-column>
          <el-table-column prop="name" label="租用人" width="180"></el-table-column>
          <el-table-column prop="returnTime" :formatter="formatTime" label="归还时间"></el-table-column>
          <el-table-column prop="statusDesc" label="状态"></el-table-column>
        </el-table>
        <pagination v-show="rentlistQuery.totalCount >= rentlistQuery.MaxResultCount" :total="rentlistQuery.totalCount"
          :page.sync="rentlistQuery.page" :limit.sync="rentlistQuery.MaxResultCount" @pagination="getRentList" />
      </div>
    </el-dialog>
    ...
    <el-table ref="multipleTable" v-loading="listLoading" :data="list" size="small" style="width: 90%;"
      ...
      <el-table-column label="状态" prop="statusDesc" sortable="custom" align="center" width="200px">
        <template slot-scope="scope">
          <el-tag type="success" effect="dark" size="small" v-if="scope.row.status == 0">
            {{ scope.row.statusDesc }}
          </el-tag>
          <el-tag type="warning" effect="dark" size="small" v-if="scope.row.status == 1">
            {{ scope.row.statusDesc }}
          </el-tag>
          <el-tag type="danger" effect="dark" size="small" v-if="scope.row.status == 2">
            {{ scope.row.statusDesc }}
          </el-tag>
        </template>
      </el-table-column>
