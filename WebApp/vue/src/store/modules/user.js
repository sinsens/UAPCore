...
const mutations = {
  ...
  SET_IS_TENANT(state, isTenant) {
    state.isTenant = isTenant
  },
  SET_TENANT_NAME(state, name) {
    ...
    return new Promise((resolve, reject) => {
      axiosMethods.getPermissions('/api/abp/application-configuration')
        .then(response => {
          ...
          commit('SET_IS_TENANT', response.currentTenant.isAvailable)
          commit('SET_TENANT_NAME', response.currentTenant.name)
